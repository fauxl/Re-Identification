...
#features_per_predire = pd.read_csv (r'C:\\Users\\FauxL\\Desktop\\Data Science\\Project\\adult.csv', usecols = ['Age', 'Workclass', 'Education',
                                                                                                                                                      #          'Marital-status', 'Occupation', 'Relationship',
                                                                                                                                                       #          'Race', 'Sex', 'Native-Country',
                                                                                                                                                        #         'Income'])
features_per_encoder = features_per_allenamento
...
for x in range(0,10):
  ...
  #  singola_feature_encoder = features_per_encoder.iloc[:, x]
   ...
   #singola_feature_predizione = pd.DataFrame(label_encoder.transform(singola_feature_predizione))
    ...
    label_encoder.fit(singola_feature_training)  # fitto l'encoder con tutte le possibili classi della feature, DI NUOVO
...
classificatore.fit(dataframe_training, classi_target.values.ravel()) #fitto con adaboost tutto dataset training
